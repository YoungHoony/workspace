...
<body>
    ...
    <pre>
        ...
        <h4>1. page (pageContext) : 현재 페이지</h4>
            -> 현재 JSP에서만 사용 가능한 객체(Servlet에서는 안됨)
        ...
        - session : 서버에 접속한 클라이언트를 나타내거나, 관련 정보를 get/set 할 수 있는 객체를
                    의미 (session 객체는 서버에서 관리함)  

        - [중요!] session은 브라우저(ex.explorer, firefox,chrome etc) 마다 하나씩 생성된다!! (새탭, 새창은 아님)
    ...
    <h2>\${XXXXScope.key} : 
        XXX 자리에 scope를 작성하면 해당 scope에서 key가 일치하는 속성을 찾아 출력 </h2>
