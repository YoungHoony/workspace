...
@SessionAttributes({"loginMember"}) // loginMember를 세션에 올리기
public class MemberController {
	
	// MemberSerivce 의존성 주입
	@Autowired
	...
	@GetMapping("logout") // /member/logout(GET)
	...
	public String signup() {
		// resources/templates/member/signup.html로 forward
	 ...
	 */
	@PostMapping("signup")
	public String signup(/*@ModelAttribute (생략가능)*/ Member inputMember, String[] memberAddress, RedirectAttributes ra) {
						//@modelAttribute == 커맨드 객체로 만드는 것 (memberAddress, id..등 Member로 불러들이는것)
		// 회원 가입 서비스 호출
		...
		return "redirect:signup"; //회원 가입 페이지 (상대 경로)
	 ...
	 * @param model : Spring에서 session scope나 request scope로 데이터를 전달하는 객체
	 * @param ra : 리다이렉트 시 request scope로 한번만 데이터 전달
	 * @return 
	 */
...
//		log.debug("memberEmail : " + memberEmail);
		...
		// (기본값) 으로 request scope인데, 
		// @SessionAttributes({"loginMember"}) 이거 때문에 Session scope로 올라가게 된다. 
		model.addAttribute("loginMember", loginMember);
