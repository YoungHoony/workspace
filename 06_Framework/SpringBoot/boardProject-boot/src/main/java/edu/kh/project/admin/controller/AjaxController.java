...
public class AjaxController {
	...
	public String selectMemberNo(int memberNo) {
		...
		/* 비동기 방식으로 응답하는 방법 ! */
		// - 비동기 통신은 페이지의 갱신(화면 새로 만들기)이 목적 X
		// - 단순히 요청에 맞는 데이터만 주고 받는 것이 목적임
		// -> 조회 결과 email을 요청한 곳으로 반환 	
		return email;
		// /templates/member01@naver.com.html (문제 발생하는중)
		
		/* *******!문제 발생!******** */
		// - email을 반환 하려는데 Controller는 반환된 값을 ViewResolver로 전달해
	...
	@ResponseBody // forward가 아니고 값 자체를 반환하기 때문에 ResponseBody를 사용 (비동기)
	...
	@ResponseBody // 비동기 통신 응답(forward/redirect가 아니라 data 자체)
	public List<Member> selectAll() {
		
		// List (Java 객체)
		// -> HttpMessageConverter가 JSON(문자열 : 그대로 출력해서 쓸 수 없기 때문에)으로 변환시켜줌
		//    (+ produces 속성을 이용해 응답 받는 JS에서 자동으로 JS 객체로 변환하도록 함) 
		// -> JS 객체
		
		
		// -> JS 객체를 만드는 과정이 필요함 (Java 객체를 그냥 쓸수없음)
		
		
		return service.selectAll();
	}
	
	// @RequestBody (요청 body부분)
	...
	/** 회원 탈퇴여부 변경
	 * @return paramMap : flag, targetNo가 담겨있는 map
	 */
