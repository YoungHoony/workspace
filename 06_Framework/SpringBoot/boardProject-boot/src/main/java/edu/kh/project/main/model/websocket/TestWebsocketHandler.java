...
// 1) TextWebSocketHandler 상속 후
//  -> afterConnectionEstablished 오버라이딩
//	   handleTextMessage 오버라이딩
//	   afterConnectionClosed 오버라이딩

// 2) Bean 등록을 하여 (Spring이 알아서 객체로 만들고 상황에 맞게 동작하게 함)

@Slf4j
@Component // Bean 등록하는 어노테이션이며 @Controller, @Service의 부모 어노테이션임)
public class TestWebsocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		for(WebSocketSession s : sessions) {
			...
			// 특정 클라이언트 메시지 -> 서버 -> 모든 클라이언트한테 보내는 것 
