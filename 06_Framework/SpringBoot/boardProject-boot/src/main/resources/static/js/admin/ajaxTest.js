...
selectMemberNo.addEventListener("click", () => {
  ...
  // GET : 조회(SELECT)
  // 쿼리스트링(querystring) : 주소에 담겨진 파라미터를 나타내는 단어 
  ...
  // 두번째 then : 첫 번째 then()에서 반환된 결과를 이용해 기능을 수행
...
btn2.addEventListener("click", () =>{
  ...
  .then( response => response.text() ) //첫번째 then : 응답이 왔을 때 수행, 응답 결과를 text로 파싱
...
btn3.addEventListener('click', () => {
  ...
  .then( (member) => {
    ...
    for(let key in member) {
      ...
      li.innerText = `${key} : ${member[key]}` // li 안에 내용 추가
  ...
  .catch( e => { // 조회 결과가 null이면 JSON 파싱에서 예외 발생
    ...
    const h4 = document.createElement("h4"); // h4 요소 생성
...
btn4.addEventListener("click", () => {
  ...
  .then( resp => resp.json()) // List객체 -> JSON Array로 변하고 -> JS 객체 배열로 변환 (배열 모양)
  .then( emailList => {
...
btn5.addEventListener("click", () => {
  ...
  .then( response => {
    ...
    // console.log(response.json()); // promise 객체가 나타남

    // json형태로 넘어온 응답 데이터(memberList)를 
    // JS 객체 형태로 변환해서 반환 
    // (첫 번째 then에서 리턴시 promise객체에 데이터만 반환 )
    return response.json(); 
  })

  .then(memberList => {
    // memberList : 첫 번째 then에서 반환된 JS 객체(변환까지 완료된 상태로 넘어옴)
    ...
    for (let member of memberList) { // 향상된 for문
      ...
      btn.addEventListener("click", e => {
        // 버튼에 속성 중 "data-flag" 값이 'N'이면 'Y', 'Y'이면 'N'을 저장
        ...
        fetch("/ajax/updateFlag", {
          method : "PUT", // PUT 방식 요청 (PUT == 수정하라는 의미를 가지고 있음)
...
btn6.addEventListener("click", () =>{
  ...
  */
  ...
  fetch("/ajax/insertMember", {
    ...
    body : JSON.stringify(member) // GET방식을 제외하고 POST,PUT, DELETE는 body에 데이터를 담아서 전달
