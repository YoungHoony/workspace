...
public class MemberController {
	...
	// 등록된 bean 중 MemberServiceImpl(상속된)을 
	// 해당 필드에 의존성 주입
	@Autowired
	...
	public String login(HttpServletRequest req) {
		...
		/* - Argument Resolver(매개 변수 해결사, 확인자)
		 * 매개변수에 원하는 타입을 선언하면 
	...
	//@PostMapping("/login") // @RequestMapping의 POST 버전 (POST 요청만 받겠다)
	public String login(@RequestParam("memberEmail") String email
		 ...
		 * 
		 *  -> 해석 : 파라미터 중 key(name 속성값)가 "memberEmail"의 value를 
		 *  	      매개변수 email에 대입
		 *  
		 * - [2] @RequestParam 생략
	...
	public String signup(Member inputMember,
		...
		// RedirectAttributes : 리다이렉트 시 값을 1회성으로 전달하는 객체 (다시 요청하면 이전 요청은 없어지는것)
