...
// Service : 비즈니스 로직 처리하는 역할(데이터 가공, DB 연결, 트랜잭션 제어)

@Service // 서비스를 역할을 명시 + bean 등록 (객체로 저장해서 spring을 관리)
public class MyPageServiceImpl implements MyPageService{
	
	@Autowired // MyPageDAO(또는 상속 관계) bean DI(의존성 주입)
	private MyPageDAO dao;
	
	@Autowired // bean DI (의존성 주입)
	// spring-security.xml 참고 (bean 저장하는게 여기에 있음)
	private BCryptPasswordEncoder bcrypt;
	
	@Transactional(rollbackFor = Exception.class) // 정상 수행 시 commit, 모든 예외 발생시 rollback한다
	@Override
	...
	public int changePw(String currentPw, String newPw, int memberNo) {
		...
		// 왜? 마이바티스 코드는 파라미터를 하나만 전달할 수 있기 때문에. 
		
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("newPw", bcrypt.encode(newPw)); // 새 비밀번호를 암호화
		map.put("memberNo", memberNo);
		
		// 4. DAO 메서드 호출 후 반환된 결과를 Controller로 반환 + @Transactional 처리도 같이 한다.
	...
	public int secession(String memberPw, int memberNo) {
		...
		// 입력 받은 비밀번호가 DB에 저장된 로그인한 회원의 비밀번호와 같다면
		// 회원 탈퇴 수행 후 결과 반환
