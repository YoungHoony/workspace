...
check1.addEventListener("click", () => {
    ...
    // 정규 표현식 객체 생성 
    // 1) new RegExp("정규표현식")
    let regEx = new RegExp("\d{3}"); // \d{3} == 연속적으로 작성된 숫자 3개를 의미
    // 2) /정규표현식/
    ...
    //정규표현식객체.exec(문자열) -> 일치하는 패턴의 문자열 반환
    //                             없으면 null
...
check2.addEventListener("click", () => {
    ...
    str = " Hello789 World"; // 띄어쓰기도 문자라서 false
    console.log(`${str} : ${regEx.test(str)} `); // false

   
    // $(달러) : 문자열의 끝을 의미 
    // \d : 아무 숫자 == [0-9]
    regEx = /\d\d\d$/; // 문자열 끝에 숫자가 3칸
    str ="123456789";
    console.log(`${str} : ${regEx.test(str)} `); // true
    ...
    /* ^, $를 같이 사용하는 경우 문자열의 시작과 끝이 정해져 있어
        문자열 안에 포함되어있다라는 의미는 사라지며,
        문자열 전체적인 형태를 제한함.  
     */
    //영어 소문자로 시작
    // 중간에 "TEST"
...
inputName.addEventListener("keyup", () => {
    ...
    // 정규식을 이용해서 검사
    if(regEx.test(value)) { // true가 나왔을 때 유효하다
...
const phoneNo = document.querySelector("#input-phone");
...
phoneNo.addEventListener("keyup", () => {
    ...
    if(regEx.test(value)) {
        ...
        phoneResult.innerText ="유효한 형식이 아닙니다"
