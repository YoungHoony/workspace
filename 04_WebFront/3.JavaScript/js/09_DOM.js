...
document.getElementById("btn1").addEventListener("click", () => {
    ...
    // 2) append("내용" 또는 노드 또는 요소) : 마지막 자식으로 내용/노드/요소로 추가
    // + 매개변수로 여러 값을 작성해 한 번에 추가 가능 
    list[11].append("1234", "abcd", "안녕");
    
    // 3) prepend("내용" 또는 노드 또는 요소) : 첫번째 자식으로 내용/노드/요소로 추가
    //+ 매개변수로 여러 값을 작성해 한 번에 추가 가능
    list[11].prepend("가가가", "나나나", "다다다");
    list[11].prepend("라라라"); //첫번째 자식으로 들어가기 때문에 맨 앞으로 된다. 
...
btn2.addEventListener("click", () => {
    ...
    // #test2를 기준으로 (부모요소)body 태그의 첫 번째 자식 요소 선택
    test2.parentElement.firstElementChild.style.color="blue";
