...
// JS 객체 다루는 방법
// checkList["KEY"]; (GET) 얻어오는 방법
// checkList["KEY"] = true; (SET) 세팅하는 방법
...
// form 태그에서는 name(signupForm)을 바로 쓸 수 있다.
...
inputId.addEventListener("input", e => {
    ...
    if(regEx.test(val)) { // 유효한 경우
        ...
        checkList["input-id"] = true;    // 유효성 검사
        ...
        checkList["input-id"] = false; //유효성 검사
...
inputPw.addEventListener("input", e =>{
    ...
    if(regEx.test(val)) { // 유효한 비밀번호 일 때
        span.innerText ="유효한 비밀번호 입니다.";
        span.classList.add("check");
        span.classList.remove("error");

        checkList["input-pw"] = true; //유효성 검사
        ...
        checkList["input-pw"] = false; //유효성 검사
    }
...
function checkPwFn(){
    ...
    if(pwVal.length == 0 || checkVal.length == 0){ // 비밀번호 미입력 시 
        ...
        checkList["input-pw-check"] = false; //유효성 검사
    ...
    if(pwVal === checkVal) {
        ...
        checkList["input-pw-check"] = true; //유효성 검사
...
signupForm.addEventListener('submit', e => {
    ...
    /* 객체 전용 향상된 for문 : for(let key in 객체명)
        -> 객체가 가지고 있는 K값을 순서대로 하나씩 key에 대입
    */

    for(let key in checkList){
        // console.log(key);

        if(!checkList[key]) { // (false인 경우) 유효하지 않은 입력이 있을 경우
