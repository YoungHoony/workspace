...
public class ExceptionService {
	...
	public void ex1() {
		...
		// BufferedReader : 빠른데 기능이 별로 없는 것 == 빨리 읽어오는 객체
		...
		try {
			...
			//  == br.readLine 문제가 많은 코드를 가지고 있기 때문에, exception처리를 해야한다. 
	...
	public void ex2() {
		 ...
		 try {
			 ...
			 // InputMismatchException 발생 가능성 있음 (숫자가 아닌 다른걸 입력했을때)
			 // 이것도 unchecked exception임 --> 꼭 예외처리 안해도 된다. 
			 ...
			 // ArithmeticExcpetion : 산술적예외
			 System.out.println("0으로 나눌 수 없습니다. ");
		 
		 } catch(InputMismatchException e) { // catch문 이후에 catch문을 연달아서 작성 가능!
			 System.out.println("잘못 입력");
		 }
		 
		 // Unchecked Exception 
		 // --> 확인하지 않은 예외, 예외 처리가 강제되지 않는 예외
	...
	public void ex4() {
		...
		Scanner sc = new Scanner(System.in);
		 ...
		 // catch(InputMismatchException e) {
			 // Unreachable : 도달할 수 없는 (절대로 실행될 수 없다는 표시)
	...
	public void ex7() {
		// throws : 메서드에서 발생한 예외를 호출한 상위 메서드로 전달하는 것 
		//          내가 못하니까 책임을 전가하는 것
		// 장점 : 한 곳에 예외를 몰아서 처리가 가능하다. == 코드 길이가 줄어든다. 
		try {
			methodA();
	...
	public void methodC() throws IOException, DataFormatException {  //부모타입의 참조 변수로 보면 된다. 
								 // 부모 타입 예외 처리 (여기서도 다형성이 적용된다) 
...
//		throw new EOFException(); // 자식 타입 예외 객체
		...
		// -> 별도 처리가 필요하다.  --> throws가 여러 예외를 던지게 함. 
		
		
		// EOFException : (End of file Exception) == IOException의 자식 
	...
	public void ex9() throws UserException {
		...
		if(input > 100) { // 입력한 숫자가 100 초과일 때 예외 발생
			//throw new UserException();  //던지기 전에 객체를 만들어야한다.
