...
public class StudentView {
	...
	public void displayMenu() {
		...
		do {
			...
			System.out.println("5. 학생 정보 수정 (인덱스 번호)");
	...
	private void selectIndex() {
		...
		// StudentSerivce로 index를 전달하여
		// 해당하는 index에 학생 객체가 있으면 그 객체의 주소를 반환
		// 그런데 index에 학생이 없거나 범위가 초과되면 null 반환
		Student s = service.selectIndex(index);   // service를 index로 전달해라
	...
	public void updateScore() {
		...
		System.out.print("수정할 학생의 인덱스 번호를 입력해주세요 : ");
		int index = sc.nextInt();
		
		// 입력 받은 인덱스에 학생이 있는지 확인
		
		Student s = service.selectIndex(index);
		//이미 존재하는 service의 selectIndex(index) 메서드를 이용 // 3번의 정보 조회에 만들어놓은거
	...
	public void selectName() {
		...
		Student[] arr = service.selectName(name); //동명이인이 있을수 있으니까. 
	...
	public void selectMaxMin() {
		...
		Student[] arr = service.selectMaxMin(); //service를 먼저 호출한다. //최고점, 최저점 학생 조회
		
		System.out.printf("최고점 : %d학년 %d반 %d번 %s \n"
				+ "국어: %d / 수학: %d / 영어: %d / 평균 : %.1f \n", arr[0].getGrade(), arr[0].getBan(), 
				arr[0].getNumber(),arr[0].getName(), arr[0].getKor(), arr[0].getMath(),
				arr[0].getEng(), ( arr[0].getKor() + arr[0].getMath() + arr[0].getEng())/3.0);
		
		System.out.println(); //개행
		
		System.out.printf("최고점 : %d학년 %d반 %d번 %s \n"
				+ "국어: %d / 수학: %d / 영어: %d / 평균 : %.1f \n", arr[1].getGrade(), arr[1].getBan(), 
				arr[1].getNumber(),arr[1].getName(), arr[1].getKor(), arr[1].getMath(),
				arr[1].getEng(), ( arr[1].getKor() + arr[1].getMath() + arr[1].getEng())/3.0);
