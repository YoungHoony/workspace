...
public class MapEx {
	 ...
	 *     * Key 역할을 하는 객체에는
	 *     	 equals(), hasCode()가 오버라이딩 되어있어야한다!
	 *      Key는 String을 제일 많이 쓴다
	 *      
	 *   2) 순서 유지 X
	 *   
	 ...
	 *     --> Value만 묶어놓으면 List와 같은 특징을 지김 ( 중복 허용)
	 *    
	 *
	 */
...
//		서브웨이클럽
		...
		// 4. V map.remove(K k) // K자료형의 k 변수명을 적어라
...
//		[롯데타워] 서울시 송파구
	...
	public void ex3() {
		...
		Map<String, Object> s1 = new HashMap<String,Object>(); //key는 string타입, value는 object타입
													//Object의 참조변수
		// Value의 타입이 Object로 제한되어있다. 
		// == 어떤 객체든 Value에 들어올 수 있다. 
		
		s1.put("grade", 3); //3(int) -> Integer로 Auto Boxing 된다
		s1.put("ban", 5);
		s1.put("number", 17);
		s1.put("name", "홍길동");
		s1.put("gender", 'M');
		...
		for(String key : set ) { //set에서 하나씩 꺼내서 반복을 하겠다라는 뜻
		System.out.printf("%s : %s \n", key, s1.get(key));
		}
		
		System.out.println("------------------------------------");
		
		// s1.put("grade", 3); == s2.setGrade(2);
		...
		// Value는 전부 Object 타입인데, 알아서 동적 binding 적용해서 자식 overriding된 것으로 잘 실행됨.  
