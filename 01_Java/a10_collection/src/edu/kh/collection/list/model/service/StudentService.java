...
public class StudentService {
	...
	// 필드								
									  //ArrayList, LinkedList는 List의 상속받은 것으로 거의 두개만 사용을 많이 한다. 
	...
	public StudentService() {
		...
		studentList.add(new Student(2, 8, 21, "김갑순", 'F', 86));
		studentList.add(new Student(1, 3, 14, "장재희", 'M', 99));
		studentList.add(new Student(4, 6, 10, "정준영", 'M', 56));
		studentList.add(new Student(2, 4, 7, "김샛별", 'F', 80));
	...
	/** 학생 추가 service 메서드
	 * @param grade //param == 파라미터, 전달인자, 전달 받은 값)
	 * @param ban
	 * @param number
	 * @param name
	 * @param gender
	...
	/** 인덱스가 일치하는 학생 1명 조회
	 * @param index
	 * @return Student 객체 주소 또는 null도 반환될 가능성이 있음. 
	 */
	public Student selectOne(int index) {
		...
		return studentList.get(index);
				//list 타입으로 List<Student> --> Student 타입이다
	}
	...
	/** 학생 성별 조회하는 Service 메서드
	 * @param gender
	 * @return searchList (검색 결과 목록)
	 */
	
	public List<Student> selectGender(char gender) {
		
		// 1. 검색 결과를 저장할 List부터 생성. (장바구니 역할이 필요함)
		List<Student> searchList = new ArrayList<Student>();
		
		// 향상된 for문 : for ( 요소 1개 참조 변수 : 배열 또는 컬렉션)
		//  -> 매 반복하다 배열 또는 컬렉션의 요소를
	...
	public List<Student> selectName(String name) {
		...
		for (Student s : studentList) {
			...
			// s.getName() == name (주소 비교--> 다른 주소이기 때문에 같을 수 거의 없다)
			// 주소가 들어가있음. 
	...
	public boolean duplicateValidation(int grade, int ban, int number, String name, char gender, int score) {
		...
		// * contains 사용 전제조건 *
		// 비교하려는 객체(클래스)에 Object의 equals()가 오버라이딩 되어있어야한다!
	...
	public List<Student> sortScore() {
		// Objects / Arrays / Collections : (s)가 붙은 것들은 유용한 기능을 모은 것/모음
		
		// Collections : 컬렉션의 유용한 기능 모음 (이 안에 정렬이라는 기능이 포함됨)
