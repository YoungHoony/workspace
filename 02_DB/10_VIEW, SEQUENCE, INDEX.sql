 ...
 * 
 *  2) FORCE | NOFORCE 옵션 : 
 *    FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성 (강제로)
...
NOMINVALUE     -- 최소값 없음
--MINVALUE 50  -- CYCLE이 돈 후 처음 시작하는 값
NOCYCLE        -- 최대값 까지 증가 후 반복 X
...
-- 먼저 NEXTVAL 수행 후 CURRVAL을 수행해야한다!

-- 다시 NEXTVAL 호출 
SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 이전 값 100에서 5씩 증가 == 105
...
FROM EMPLOYEE e WHERE SALARY > 3000000;
