...
-- 하동운, 이오리는 DEPT_CODE NULL이기 때문에
-- DEPT_ID 중 일치하는 값이 없어서 합친(JOIN) 결과에 포함 X



-- 2) 오라클 (JOIN이라는 단어를 작성하지 않음)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE
...
FROM EMPLOYEE, DEPARTMENT 
WHERE DEPT_CODE = DEPT_ID 
AND DEPT_TITLE = '해외영업2부' -- 오라클 방식 조인 시 추가 조건은 AND를 사용
ORDER BY EMP_ID;
...
-- LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- OUTER JOIN (23행, 하동운, 이오리 추가)
-- RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- OUTER JOIN (24행, 마케팅부, 국내영업부, 해외3부)
FULL JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- FULL JOIN (26행)

--********************* 오라클 방식은 FULL JOIN 불가************************
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT 
WHERE DEPT_CODE(+) = DEPT_ID(+);
...
-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCT)  -- 잘 사용 X
...
-- 4. 비등가 조인(NON EQUAL JOIN) -- 잘 사용 X
...
-- 사원의 급여가 SAL_LEVEL에 작성된 최소 (MIN_SAL) ~ 최대(MAX_SAL) 범위의
-- 급여가 맞을 때만 조인


---------------------------------------------------------------------------------------------------------------

-- 5. 자체 조인(SELF JOIN) ****************************** 많이 사용

-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음

-- 사번, 이름, 사수번호, 사수의 이름 조회
-- EMP_ID, EMP_NAME, MANAGER_ID 

-- **TIP! 하나의 테이블만 생각하지 말고, 똑같은 테이블이 두개가 있다고 생각해라.
...
-- EMPLOYEE, DEPARTMENT, LOCATION 세 개의 테이블을 조인. 
...
FROM EMPLOYEE e
NATURAL JOIN JOB j -- JOIN JOB USING(JOB_CODE)
WHERE EMP_NAME LIKE '%형%';


-- 3. 해외영업 1부, 2부에 근무하는 사원의 
-- 사원명, 직급명, 부서코드, 부서명을 조회하시오. 사번 오름차순으로 조회
...
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY*12*(NVL(BONUS,0)+1) 연봉
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
...
SELECT EMP.EMP_NAME 사원명, EMP.DEPT_CODE 부서코드, MGR.EMP_NAME 동료이름
FROM EMPLOYEE EMP
JOIN EMPLOYEE MGR ON (EMP.DEPT_CODE = MGR.DEPT_CODE)
