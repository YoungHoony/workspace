...
/* 자료형
    1) NUMBER : 숫자형(정수, 실수) -- 10^38제곱까지
    2) CHAR(크기) : 고정길이 문자형 (2000BYTE)  -- 자바에서는 STRING(문자열)과 같은 것이다. 
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        
    3) VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) -- CHAR에 비해서 2배 차이.
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        
    4) DATE : 날짜 타입
    5) BLOB : 대용량 이진 데이터 (4GB) (Byte Long Of Byte)
    6) CLOB : 대용량 문자 데이터 (4GB) (Character Long Of Byte) -- 문자만 4기가를 저장할 수 있음. -- 블로그 같은 
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    -- 1) USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건 (UNIQUE 제약 조건 )
    -- 2) USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,
    					-- CONSTRAINT 제약조건명 제약조건종류
    					-- 컬럼 레벨 제약 조건(제약 조건 이름 지정됨)
    ...
    -- 3)UNIQUE(USER_ID) -- 제약조건(컬럼명) --> 제약조건명 미지정 
    /*4)*/ CONSTRAINT USER_ID_U UNIQUE(USER_ID) --> 제약조건명 지정
);
...
-- ORA-00001: 무결성 제약 조건(A230724_JYH.SYS_C0013067)에 위배됩니다
-- SYS_C0013067 : 제약조건 이름 미설정 시 자동으로 지정되는 이름 
...
--> USER_NO가 다름
--> ORA-00001: 무결성 제약 조건(A230724_JYH.USER_ID_NAME_U)에 위배됩니다
--> USER01, 홍길동 값이 같기 때문에
...
CREATE TABLE USER_USED_PK(
	-- 컬럼 레벨로 PK 지정
    --USER_NO NUMBER PRIMARY KEY,  --1) 컬럼 레벨, 이름 지정 X
    --USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, --2) 컬럼 레벨, 이름 지정 0 
    
	USER_NO NUMBER, -- 테이블 레벨 지정용 컬럼 (테이블 마지막 줄에 작성)
    
    USER_ID VARCHAR2(20) UNIQUE, -- USER_ID ==PRIMARY KEY로 하면 문제가 생길수있음. 
...
--> 기본키 중복으로 오류
--> ORA-00001: 무결성 제약 조건(A230724_JYH.USER_NO_PK)에 위배됩니다
--> UNIQUE랑 NOT NULL 모든 조건을 다 가지고 있음. 
--> UNIQUE 위배

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--> ORA-01400: NULL을 ("A230724_JYH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
--> NOT NULL 위배
...
INSERT INTO USER_USED_PK2
VALUES(1, 'user02', 'pass02', '이순신', '남', '010-5678-9012', 'lee123@kh.or.kr');
-- 정상적으로 된다. 복합키는 모든 조건의 값이 맞아야 위배
...
-- ORA-00001: 무결성 제약 조건(A230724_JYH.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("A230724_JYH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  -- 컬럼 레벨
  --GRADE_CODE NUMBER REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨 (FOREIGN KEY), 이름X
  --GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK1 
  --			REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨, 이름 O
  GRADE_CODE NUMBER,
...
--> ORA-02291: 무결성 제약조건(A230724_JYH.GRADE_CODE_FK1)이 위배되었습니다
--           - 부모 키가 없습니다

-- 참조하는 테이블 = 자식 테이블
-- 참조를 당하는 테이블 = 부모 테이블 

/* JOIN : 두 테이블에서 같은 값을 가지고 있는 컬럼을 기준으로 두 테이블을 연결
...
DELETE FROM USER_GRADE 
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(A230724_JYH.GRADE_CODE_FK1)이 위배되었습니다
-- - 자식 레코드가 발견되었습니다
-- 작동 안함!
...
SELECT * FROM USER_USED_FK3; -- 홍길동, 이순신 삭제 (10 참조하던 행 전체 삭제)
...
-- ORA-02290: 체크 제약조건(A230724_JYH.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST (
	USER_NO NUMBER CONSTRAINT PK_USER_TEST PRIMARY KEY,
 	...
 	PNO VARCHAR2(60)  CONSTRAINT NN_PNO NOT NULL, -- NOT NULL은 컬럼레벨에서만 가능함
...
VALUES(1, 'USER1', 'USER1', '990101-1234566', '남', '010-1234-5678', '서울시 강남구','Y'  );

INSERT INTO USER_TEST 
VALUES(2, 'USER2', 'USER2', '990201-1234566', '여', '010-5678-9123', NULL ,'Y'  );

INSERT INTO USER_TEST 
VALUES(3, 'USER3', 'USER3', '990301-1234566', '남', NULL , '서울시 강남구',  'N'  );

INSERT INTO USER_TEST 
VALUES(4, 'USER4', 'USER4', '990401-1234566', '여', '010-4567-8912', '서울시 강남구', 'N'  );

INSERT INTO USER_TEST 
VALUES(5, 'USER5', 'USER5', '990501-1234566', '남', '010-3456-7891', '서울시 강남구', 'Y' );
...
ALTER TABLE EMPLOYEE ADD -- 무언갈 추가해서 EMPLOYEE 테이블을 변경할거다. 
CONSTRAINT SAL_GRADE_FK -- 제약 조건 이름 지정
FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
	-- FK제약조건 추가 (EMPLOYE.SAL_LEVEL 컬럼이 SAL_GRADE의 PK 참조)
