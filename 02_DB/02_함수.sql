...
-- 이메일 길이 오름차순 조회
/*3*/SELECT EMP_NAME 사원명, EMAIL "이메일", LENGTH(EMAIL) AS "이메일 길이" 
/*1*/FROM EMPLOYEE
/*2*/WHERE LENGTH(EMAIL) <= 12
--WHERE "이메일 길이" <= 12; -- 해석 순서가 2번째이기때문에, 이메일 길이가 나올수없다.
/*4*/ORDER BY "이메일 길이" ;--LENGTH(EMAIL);

-- INSTR(문자열 | 컬러명, '찾을 문자열' [, 찾을 시작 위치 [, 순번]]) -- [] 안은 꼭 안써도 된다. 필요하면 쓴다
...
SELECT REPLACE(NATIONAL_NAME, '한국', '대한민국') 나라이름 
FROM NATIONAL;
...
-- ROUND(숫자 | 컬러명 [, 소수점 위치]) : 반올림
-- 소수점 위치 지정 X : 소수점 첫째 자리에서 반올림 -> 정수 표현 
-- 소수점 위치 지정 O 
   -- 1) 양수 : 지정된 위치의 소수점 자리까지 표현
   -- 2) 음수 : 지정된 위치의 정수 자리까지 표현
   
-- 소수점을 업애고 싶을때, 만 단위나 조금 더 보고싶을때
SELECT 123.456,
	ROUND(123.456), -- 123
	ROUND(123.456, 1), -- 123.5 (소수점 첫째자리까지 나오게한다. 2번째자리에서 반올림)
	ROUND(123.456, 2), -- 123.46 (3번째자리에서 반올림)
...
SELECT -123.5,
	TRUNC(-123.5), -- -123 (0.5버림)
	TRUNC(-123.5, -1), -- -120 (일의자리에서 버림)
	FLOOR(-123.5), -- -124 (0.5내림)
	TRUNC(2.5), -- 2 (.5버림)
	FLOOR(2.5) -- 2 (.5내림)
FROM DUAL;
...
-- MONTHS_BETWEEN() : 윤년 계싼이 알아서 포함되어 있음

SELECT SYSDATE, SYSDATE + 31 "한달 후" FROM DUAL;
SELECT SYSDATE, SYSDATE + 61 "두달 후" FROM DUAL;
SELECT SYSDATE, SYSDATE + 92 "세달 후" FROM DUAL;

-- ADD_MONTHS(날짜, 숫자) : 날짜를 숫자만큼의 개월수를 더하여 반환
SELECT ADD_MONTHS(SYSDATE, -1) "한달 전", SYSDATE, ADD_MONTHS(SYSDATE,1) "한달 후", ADD_MONTHS(SYSDATE, 10) "10개월 후" 
FROM DUAL;
...
-- 포맷의 역할 : 문자열 또는 숫자의 패턴이 날짜의 어떤 부분을 가리키고 있는지를 나타냄
...
/*** 연도 패턴 Y, R의 차이점 ***/
-- 연도가 두자리만 작성되어있는 경우 
-- 50 미만 (0~49) : Y,R 둘 다 누락된 연도 앞부분에 현재 세기 (21C == 2000년대) 추가
-- 50 이상 (50~99) : Y : 현재 세기(2000년대) 추가
--					R : 이전 세기(1900년대) 추가

-- 50 미만
SELECT TO_DATE('49-11-25', 'YY-MM-DD') "YY", -- 2049-11-25
	   TO_DATE('49-11-25', 'RR-MM-DD') "RR"  -- 2049-11-25
FROM DUAL;
	  
-- 50 이상	   
SELECT TO_DATE('50-11-25', 'YY-MM-DD') "YY", -- 2050-11-25
	   TO_DATE('50-11-25', 'RR-MM-DD') "RR"  -- 1950-11-25
FROM DUAL;	  
...
-- 컬러명 | 계산식의 값이 일치하는 조건이 있으면 
-- 해당 조건 오른쪽에 작성된 결과가 반환된다. 

-- 모든 조건 불만족 시 [, 아무것도 만족 X]이 반환됨.
...
-- EMPLOYEE 테이블에 존재하는 남자, 여자 사원의 수 (SUM으로)
