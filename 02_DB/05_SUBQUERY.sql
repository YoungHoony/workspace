...
/*2*/SELECT EMP_NAME, DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE = (/*1*/SELECT DEPT_CODE FROM EMPLOYEE
				   WHERE EMP_NAME = '노옹철');
...
-- 1) 전 직원의 평균 급여 조회하기
SELECT AVG(SALARY) FROM EMPLOYEE e ; -- 약 3047663원
...
-- GROUP BY 할 때는 SELECT에서 컬럼 그룹 함수가 없는 것을 넣어야한다. 
...
FROM EMPLOYEE e 
WHERE SALARY IN (SELECT MAX(SALARY) FROM EMPLOYEE e 
GROUP BY DEPT_CODE);
-- IN 괄호 안에 SELECT 결과 7행이 다 들어가게 된다. 
...
-- 방법 1 : SET OPERATION (집합 연산자)	중 UNION 이용
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사수' "구분"
FROM EMPLOYEE e 
NATURAL JOIN JOB j 
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > (SELECT MIN(SALARY) FROM EMPLOYEE e 
			  JOIN JOB USING(JOB_CODE)
			  WHERE JOB_NAME = '과장');
...
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > ANY (SELECT SALARY FROM EMPLOYEE e 
			  JOIN JOB USING(JOB_CODE)
			  WHERE JOB_NAME = '과장');
			 -- 3760000, 2200000(최소), 2500000
...
NVL((SELECT EMP_NAME 
     FROM EMPLOYEE SUB
     WHERE SUB.EMP_ID = MAIN.MANAGER_ID),'없음') "사수 이름"
FROM EMPLOYEE MAIN; 
...
SELECT 사번, 이름 FROM
	...
	-- 다른 SELECT문의 테이블 역할 (인라인 뷰) 다른 SELECT문 안에서 볼수만 있는 것
);
...
WHERE ROWNUM BETWEEN 1 AND 10;  -- 1이 포함되어서 조회는 잘 된다. 
--WHERE ROWNUM BETWEEN 2 AND 10;  -- 1이 포함되지 않아서 조회 X
...
--> WITH 사용 버전
-- 서브 쿼리를 메인쿼리에서 TOP_SALARY라고 부르겠다고 정의
...
/* 1. 테이블 구조 파악
 * 
 * 2. SELECT 해석 순서 (잘 이해해야함)
 *    + 별칭 사용이 가능한 부분
 *    EX) ORDER BY절에서는 SELECT절에서 해석된 별칭 사용이 가능
 *    EX) 인라인 뷰에서 지정된 별칭을 메인 쿼리에서도 똑같이 사용해야된다. 
 * 
