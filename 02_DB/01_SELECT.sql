...
-- EMPLOYEE 테이블 모든 행, 모든 컬럼 조회하기
-- * : 모든, 모두, ALL, 전부라는 뜻을 가짐
...
-- 해석 순서
/*3번째*/ SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE 
/*1번째*/ FROM EMPLOYEE  
/*2번째*/ WHERE SALARY > 3000000;
...
-- 컬럼명 NOT BETWEEN a AND b : a이상 b이하이면 FALSE 
--> a 미만, b초과면 TRUE
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY
FROM EMPLOYEE 
...
--> 11행이 조회되야 되는데, 현재 9행만 조회되었다. 
--> 왜?? DEPT_CODE가 NULL 행이 2행이 있어서 그렇다.

-- !!!!!!!!!!!!!!!NULL은 비교, 논리 연산할 때 연산의 대상이 되지 않음!!!!!!!!!!!!!!
...
-- IS NOT NULL  : 컬럼 값이 NULL이면 FALSE
...
-- EMPLOYEE 테이블에서 이름에서 '하'가 포함되는 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE 
WHERE EMP_NAME LIKE '%하%';

-- EMPLOYEE 테이블에서 이름이 '전' 시작, '돈으로 끝나는 사원의 사번, 이름 조회
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE 
...
-- EMPLOYEE 테이블에서 이메일의 아이디 (@ 앞의 글자)의 글자수가 5글자인 사원의 이름, EMAIL 조회
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE 
WHERE EMAIL LIKE '_____@%';

-- EMPLOYEE 테이블에서 이메일의 아이디 중 '_' 앞 쪽 단어의 수가 3글자인 사원의 사번, 이름, EMAIL 조회
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE 
WHERE EMAIL LIKE '____%';-- 이메일 4글자 이상

--> 문제점 : 와일드 카드 표기법 '_' 와 
--  	    기준으로 삼을 문자열 '_'가 둘 다 와일드 카드로 해석
...
-- 날짜 형식의 문자열이기 때문에
-- HIRE_DATE (DATE 타입)	비교시 자동으로 형변환(PARSING :종료자체가 달라지는 것)이 진행된다. 
...
-- EMPLOYEE 테이블에서 급여 오름 차순으로 이름, 급여를 조회

SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY ASC; -- ASC(ASCENDING) : 오름차순 

-- EMPLOYEE 테이블에서 급여 내림 차순으로 이름, 급여를 조회

SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC; -- DSC(DESCEDING) : 내림차순 
...
-- EMPLOYEE 테이블에서 급여가 300만 이상, 600만 미만
-- 사원의 사번, 이름, 급여를 이름 내림 차순으로 조회
...
/* ORDER BY절에서는 계산식, 함수식 작성도 가능 */
-- EMPLOYEE 테이블에서 이름, 연봉을 연봉 내림차순으로 조회
SELECT EMP_NAME, SALARY*12
FROM EMPLOYEE 
ORDER BY SALARY*12 DESC; 

/* ORDER BY 절에서는 별칭 작성도 가능! (해석 순서 중요!!) */
-- 연봉이 3500만 이상의 사원의 
...
/* 정렬시 컬럼값이 NULL 행 정렬 방법 지정*/
-- 모든 사원의 이름, 전화번호 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE
--ORDER BY PHONE NULLS LAST; -- 오름차순 -> NULL은 마지막에 배치 
--ORDER BY PHONE NULLS FIRST; -- 오름차순 --> NULL이 처음 부분에 배치
ORDER BY PHONE DESC NULLS FIRST; -- 내림 차순 -> NULL이 처음부분에 배치 (기본)
ORDER BY PHONE DESC NULLS LAST; -- 내림 차순 -> NULL을 마지막에 배치 
