...
/*3*/SELECT DEPT_CODE 부서코드/* 묶인 그룹의 이름*/, SUM(SALARY) "급여 합"/*그룹별 급여 합*/
/*1*/FROM EMPLOYEE 
...
-- 부서코드 순으로 정렬
SELECT DEPT_CODE 부서코드, COUNT(BONUS) "보너스 받는 사원의 수" /*BONUS가 NULL인 행 빼고 카운트 */
FROM EMPLOYEE 
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
...
SELECT DEPT_CODE 부서코드,ROUND(AVG(SALARY)) 평균
FROM EMPLOYEE 
...
/*5*/SELECT DEPT_CODE, ROUND(AVG(SALARY)) "급여 평균"
/*1*/FROM EMPLOYEE e 
...
WHERE SUBSTR(EMP_NO,1,1) = '7'
	-- == WHERE SUBSTR(EMP_NO,1,1) = '7'
	-- == WHERE EMP_NO LIKE '7%'  -- 7로 시작해서 뒤는 아무거나 숫자
