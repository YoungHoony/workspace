...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		Connection conn = null; // null == 참조하는 객체가 없다. 
		...
		try {
			// 2. DriverManage를 이용해 Connection 객체 생성 
			
			
			// 2-1) Oracle JDBC Driver 객체를 메모리에 로드(적재) 하기
			Class.forName("oracle.jdbc.driver.OracleDriver");
			// class.forName(클래스명) : 클래스를 읽어서 메모리 등록 (객체화를 한다 == 객체를 만든다)
			...
			// Connection 객체 생성 확인 (오류 안나면 오타없이 작성한 것이다)
